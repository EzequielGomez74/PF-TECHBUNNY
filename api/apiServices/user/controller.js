...
async function getQR(user_id){ //ACA GENERAMOS EL QR DEL USER QUE QUIERE ACTIVAR 2FA
  var secret = speakeasy.generateSecret({
    name: "TechBunny_TEST"
  });
  console.log(secret);
  const response = qrcode.toDataURL(secret.otpauth_url, function(err,data){  // RETORNA UNA IMAGEN QR PARA PONER EN ETIQUETA IMG COMO SRC <img src= ${response} />
...
async function compareGoogleAuth(user_id, token){   // FUNCION QUE COMPARA CODIGO ENVIADO POR EL USER CON CODIGO DE GOOGLE AUTH -
  ...
  if(!response.googleAuth && response.verified)     // VERIFICAMOS SI EL USER TIENE googleAuth ACTIVADO (TRUE O FALSE), SI ES FALSE, SE LO ACTIVAMOS
